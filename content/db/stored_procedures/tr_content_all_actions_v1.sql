...
BEGIN
    ...
    IF (TG_OP = 'UPDATE') THEN
        IF (NEW.data_field_id IS NULL) THEN
            DECLARE
                id_person_group     BIGINT;
                id_entity           BIGINT := NEW.entity_id;
                pg_array            BIGINT[] := '{}';
                new_pg_array        BIGINT[] := '{}';
                no_label            VARCHAR := lower(NEW."VARCHAR");
            BEGIN
...
--                     WHERE entity_id = ' || id_entity;
                EXECUTE sp_update_so_entry('sort_order',no_label,'{' || array_to_string(new_pg_array,',') || '}',id_entity);
            END;
        ELSE
...
--                     content_data :=  lower(NEW."VARCHAR");
                END IF;
...
--                     content_data :=  lower(CAST(NEW."INTEGER" AS VARCHAR));
                END IF;
...
--                     content_data :=  lower(CAST(NEW."TIME" AS VARCHAR));
                END IF;
...
--                     content_data :=  lower(CAST(NEW."BOOLEAN" AS VARCHAR));
                END IF;
...
--                     content_data :=  lower(CAST(NEW."NUMERIC" AS VARCHAR));
                END IF;
            END IF;

--             IF (content_data IS NOT NULL) THEN
--                 content_data_array :=  sp_create_content_data_array(content_data);
--                 noe := array_upper(content_data_array,1);
--                 FOR i IN 1 .. noe LOOP
--                     DELETE FROM rel_search_word_entity
--                     WHERE entity_id = NEW.entity_id
--                     AND search_word_id = (
--                         SELECT id FROM search_words
--                         WHERE search_word = content_data[i]
--                         AND language_id = ANY (NEW.language_id)
--                     );
--                     EXECUTE sp_insert_search_entry(content_data_array[i],NEW.entity_id,NEW.language_id,NEW.data_field_id);
--                 END LOOP;
--             END IF;
        END IF;
        RETURN NEW;
            ...
            BEGIN
                ...
                IF (nor = 0) THEN
                    EXECUTE 'INSERT INTO ' || quote_ident(no_table) || ' (entity_id,' || quote_ident(no_label) || ') VALUES ('
                        || id_entity || ',' || quote_literal('{551,552,553}') || ')';
                ELSEIF (nor > 0) THEN
--                     EXECUTE sp_update_so_entry('sort_order',no_label,'{' || array_to_string(new_pg_array,',') || '}',id_entity);
                    EXECUTE 'UPDATE sort_order SET ' || quote_ident(no_label) || ' = ' || quote_literal('{551,552,553}')
                        || 'WHERE entity_id = ' || 1149;--id_entity;
                END IF;
            END;
        ELSE
...
--             END LOOP;
    ...
    ELSEIF (TG_OP = 'DELETE') THEN
        IF (OLD.data_field_id IS NULL) THEN
            DECLARE
                id_person_group     BIGINT;
                ...
                no_label            VARCHAR := lower(OLD."VARCHAR");
            BEGIN
...
--                 IF (OLD."VARCHAR" IS NOT NULL) THEN
--                     content_data :=  lower(OLD."VARCHAR");
--                 ELSEIF (OLD."INTEGER" IS NOT NULL) THEN
--                     content_data :=  lower(CAST(OLD."INTEGER" AS VARCHAR));
--                 ELSEIF (OLD."TIME" IS NOT NULL) THEN
--                     content_data :=  lower(CAST(OLD."TIME" AS VARCHAR));
--                 ELSEIF (OLD."BOOLEAN" IS NOT NULL) THEN
--                     content_data :=  lower(CAST(OLD."BOOLEAN" AS VARCHAR));
--                 ELSEIF (OLD."NUMERIC" IS NOT NULL) THEN
--                     content_data :=  lower(CAST(OLD."NUMERIC" AS VARCHAR));
--                 END IF;
...
--                     AND search_word_id = (
--                         SELECT id FROM search_words
--                         WHERE search_word = content_data_array[i]
--                         AND language_id = ANY (NEW.language_id)
--                     );
--                 END LOOP;
            ELSE
                IF (sp_check_so_column(no_table,no_label)) THEN
                    EXECUTE 'DELETE FROM ' || quote_ident(no_table)
                        ||' WHERE entity_id = ' || OLD.entity_id;
                END IF;
                -- delete entries in relation table search_words - entities
--                 DELETE FROM rel_search_word_entity
--                 WHERE entity_id = OLD.entity_id;
            END IF;
        END IF;
